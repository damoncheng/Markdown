...
var CREATE_EXTENSION_ELEMENT_ACTION = 'create-extension-element',
    UP_EXTENSION_ELEMENT_ACTION = 'up-extension-element',
...
module.exports = function(element, bpmnFactory, options, translate) {
    ...
    var upElement = options.upExtensionElement,
        canUp = typeof upElement === 'function';
    ...
    return {
        ...
        html: '<div class="bpp-row bpp-element-list" ' +
                ...
                (canRemove ? '<button class="clear" ' +
                                ...
                                '<span>-</span>' +
                            '</button>' : '') +
                (canUp ? '<button class="up" ' +
                            'id="cam-extensionElements-up-' + id + '" ' +
                            'data-action="upElement" ' +
                            'data-disable="disableUp">' +
                        '<span>u</span>' +
                    '</button>' : '') +
                '</div>' +
            '</div>',
        ...
        set: function(element, values, node) {
            ...
            else if (action.id === UP_EXTENSION_ELEMENT_ACTION) {
                console.log("....here")
                return upElement(element, extensionElements, action.value, action.idx, node);
            }
        ...
        upElement: function(element, node) {
            ...
            //selectBox.removeChild(selectBox.options[selection.idx]);
            var cur_element = selectBox.options[selection.idx];
            var pre_element = selectBox.options[selection.idx - 1];
    
            console.log("cur_element...... : ", cur_element);
            selectBox.removeChild(selectBox.options[selection.idx - 1]);

            //selectBox.insertBefore(cur_element,pre_element);
    
            // update select box size
            //initSelectionSize(selectBox, selectBox.options.length);

            console.log("selectBox.options : ", selectBox.options);
    
            this.__action = {
                id: UP_EXTENSION_ELEMENT_ACTION,
                value: selection.value,
                idx: selection.idx
            };
        ...
        disableUp: function(element, entryNode, node, scopeNode) {
            return (getSelected(entryNode, id) || {}).idx <= 0;
        },
